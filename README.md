...
`quick_query()` preserves whatever column names PDO would report for the caller's select list. For explicit aliases like `select 1 as id`, that means string keys such as `'id'`. For an unaliased SQLite scalar select like `select 1`, PDO reports the column name as `1`, so the row shape is `[[1 => new \TypeDb\SqlValue\SqlInteger(1)]]`. Internal SQLite casts used to preserve float parameter types do not change those logical result names.
